...
def stream_send(args: argparse.Namespace) -> None:
    ...
    with open(out_path, "wb") as f_out:
        for chunk in SendStream(args.file, sender=sender, receiver=receiver_card):  # type: ignore[union-attr]
            f_out.write(chunk)  # type: ignore[arg-type]
...
def stream_receive(args: argparse.Namespace) -> None:
    ...
    with open(args.payload, "rb") as f_in, open(out_path, "wb") as f_out:
        for chunk in ReceiveStream(f_in, receiver=receiver):  # type: ignore
            f_out.write(chunk)
